...
function markVector(ocean, vector) {
  if (vector.x0 === vector.x1) {
    ...
    const increment = { x: vector.x1 > vector.x0 ? 1 : -1, y: vector.y1 > vector.y0 ? 1 : -1 }
    for (let x = vector.x0, y = vector.y0; x != vector.x1 && y != vector.y1; x = x + increment.x, y = y + increment.y) {
      markTile(ocean, x, y)
    }
    markTile(ocean, vector.x1, vector.y1)
  }
}
